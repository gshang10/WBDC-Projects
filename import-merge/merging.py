...
# Matches every name in OT with the list of names in SF
...
sf_name_to_id = salesforce_df["Cleaned Name"].drop_duplicates(subset=["Cleaned Name"]).set_index("Cleaned Names")["Applicant Contact: 18-Digit Account ID"]
sf_name_to_email = salesforce_df["Cleaned Name"].drop_duplicates(subset=["Cleaned Name"]).set_index("Cleaned Names")["Cleaned Email"]
